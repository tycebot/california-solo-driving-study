...
To explain including all three predictor variables (log population, region, and ethnicity) in the final model, I compared the full model against two reduced candidate models. The two candidate models were the full model excluding ethnicity, and the full model excluding region. The models were compared using adjusted $R^2$ and Akaike Information Criterion (AIC), which balance model fit against complexity. The full model has an adjusted $R^2$ of 0.96 and an AIC value of 519. For the model excluding ethnicity the adjusted $R^2$ is 0.32 and the AIC is 765. For the model excluding region the adjusted $R^2$ is 0.41 and the AIC is 746. The higher adjusted $R^2$ and lower AIC values for the full model versus the candidate models indicates that both ethnicity and region provide additional explanatory power beyond population alone. The subsequent F-tests on ethnicity and region coefficients (presented in Results) provide additional evidence supporting their inclusion.
...
# Get r-squared values for full model
...
#Get r-squared values for reduced models
no_race_rsquared<-summary(reduced_model_race)$adj.r.squared
no_region_rsquared<-summary(reduced_model_region)$adj.r.squared

#Get AIC values
full_model_AIC<-AIC(full_model)
no_race_AIC<-AIC(reduced_model_race)
no_region_AIC<-AIC(reduced_model_region)
# F tests
...
The model's weighted R-squared value is `r round(full_r_squared, 3)`, indicating that approximately `r round(full_r_squared * 100, 1)`% of the variation in drive-alone percentages is explained by race/ethnicity, region, and population size.The estimated intercept is $b_0=$ `r round(coefficients(full_model)[1], 3)`. In other words the expected solo commuting rate of the reference group(african americans in the bay area) if log(pop_total) = 0 is `r round(coefficients(full_model)[1], 1)`%. The log population coefficient is $\hat{\beta}_1 =$ `r round(coef(full_model)["log(pop_total)"], 3)`. This indicates that for a 1% increase in population, the expected percentage of workers driving alone changes by approximately `r round(coef(full_model)["log(pop_total)"]/100, 4)` percentage points.
